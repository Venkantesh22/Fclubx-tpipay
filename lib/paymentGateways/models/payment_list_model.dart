...
List<PaymentData> getPaymentList({bool includeCash = true, bool includeWallet = true}) {
  List<PaymentData> list = [];

  if (includeCash) {
    list.add(PaymentData(
      id: PaymentMethods.PAYMENT_METHOD_CASH,
      icon: ic_payment,
      paymentMethod: locale.cash,
    ));
  }
  ...
  if (includeWallet) {
    list.add(PaymentData(
      id: PaymentMethods.PAYMENT_METHOD_FROM_WALLET,
      icon: appStore.isDarkMode ? ic_wallet_balance : ic_wallet,
      paymentMethod: locale.wallet,
    ));
  }
