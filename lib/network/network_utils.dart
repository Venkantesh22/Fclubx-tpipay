...
Map<String, String> buildHeaderTokens({
  ...
  if (appStore.isLoggedIn &&
    ...
    if (token.isNotEmpty) {
      ...
      setValue(SharedPreferenceConst.TOKEN, '');
      userStore.setToken('');
      appStore.setToken('');
    }
  }
...
Future<Response> buildHttpResponse(
  ...
  // Check network connectivity using our NetworkService
  try {
    ...
    if (networkService.isConnected.value) {
      ...
      if (method == HttpMethodType.POST) {
        ...
        response =
            await delete(url, headers: headers).timeout(Duration(seconds: 20));
      } else if (method == HttpMethodType.PUT) {
        response = await put(url, body: jsonEncode(request), headers: headers)
            .timeout(Duration(seconds: 20));
      } else {
        response =
            await get(url, headers: headers).timeout(Duration(seconds: 20));
      }
    ...
    if (e == errorInternetNotAvailable) {
      throw errorInternetNotAvailable;
    }
    throw locale.somethingWentWrong;
  }
}
...
Future handleResponse(Response response,
  ...
  if (response.statusCode.isSuccessful()) {
    ...
    if (response.body.isJson()) {
      Map body = jsonDecode(response.body);
      throw body['message'] ?? locale.badRequest;
    } else {
    ...
    if (response.body.isJson()) {
      Map body = jsonDecode(response.body);
      throw body['message'] ?? locale.forbidden;
    } else {
...
Future<bool> reGenerateToken() async {
  ...
  if (getBoolAsync(SharedPreferenceConst.IS_LOGGED_IN)) {
    if (userData.loginType == LoginTypeConst.LOGIN_TYPE_OTP) {
      request = {
        CommonKey.username: userData.username,
        UserKeys.password: userData.password,
        UserKeys.mobile: userData.mobile,
        // UserKeys.userType: EmployeeKeyConst.LOGIN_TYPE_USER,
        CommonKey.loginType: userData.loginType,
        UserKeys.firstName: userData.firstName,
        UserKeys.lastName: userData.lastName,
      };
    } else if (userData.loginType == LoginTypeConst.LOGIN_TYPE_GOOGLE ||
        userData.loginType == LoginTypeConst.LOGIN_TYPE_APPLE) {
      request = {
        UserKeys.profileImage: userData.profileImage,
        UserKeys.email: userData.email,
        UserKeys.password: userData.email,
  ...
  } else if (getStringAsync(SharedPreferenceConst.USER_EMAIL).isNotEmpty &&
      getStringAsync(SharedPreferenceConst.USER_PASSWORD).isNotEmpty) {
    request = {
  ...
  if (request != null) {
    final loginType = request[CommonKey.loginType];
    final isSocialLogin = loginType == LoginTypeConst.LOGIN_TYPE_GOOGLE ||
        loginType == LoginTypeConst.LOGIN_TYPE_APPLE;

    await loginUser(request,
            isSocialLogin: isSocialLogin, isRegenerateToken: true)
        .then(
      (value) {
        result = true;
      },
...
Future<void> handleFailRegenerateToken() async {
  ...
  setValue(SharedPreferenceConst.IS_LOGGED_IN, false);
  setValue(SharedPreferenceConst.CURRENT_USER_DATA, userData.toJson());
