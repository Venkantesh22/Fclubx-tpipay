...
Future<void> commonLaunchUrl(String address, {LaunchMode launchMode = LaunchMode.externalApplication}) async {
  await launchUrl(Uri.parse(address), mode: launchMode).then((value) => null).catchError((e) {
    ShowToast.showError('${locale.invalidUrl}: $address');
  });
}
...
PreferredSizeWidget commonAppBarWidget(BuildContext context, {double? titleSpacing, bool isCenterTitle = true, String? title, double? appBarHeight, bool? showLeadingIcon, bool? roundCornerShape, List<Widget>? actions}) {
  ...
  return PreferredSize(
    ...
    child: AppBar(
      ...
      leading: !showLeadingIcon.validate()
          ? Offstage()
          : BackWidget(onPressed: onLeadingIconPressed),
      elevation: 0,
      actions: actions,
      shape: roundCornerShape.validate()
...
(String, Color) getBranchIsOpen({required String startTime, required String endTime, bool isHoliday = false, bool isFestival = false}) {
...
void serviceCommonBottomSheet(BuildContext context, {required Widget child, String? title, bool showCloseButton = true}) {
  CommonBottomSheet.show(context, title: title, child: child, showCloseButton: showCloseButton);
}
