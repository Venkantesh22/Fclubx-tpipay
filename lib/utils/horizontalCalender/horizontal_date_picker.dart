...
class _HorizontalDatePickerWidgetState extends State<HorizontalDatePickerWidget> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      widget.datePickerController.scrollToSelectedItem(widget.selectedDate);
    });
  }

  void didUpdateWidget(HorizontalDatePickerWidget oldWidget) {
    super.didUpdateWidget(oldWidget);
    _init(widget.datePickerController, widget.widgetWidth, widget.width, widget.startDate, widget.endDate, widget.selectedDate);
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: ListView.builder(
        ...
        controller: widget.datePickerController.scrollController,
        itemBuilder: (context, index) {
          var dateTime = widget.datePickerController.startDate.add(Duration(days: index));
  ...
  DateItemState _getDateTimeState(DateTime dateTime) {
    if (_isSelectedDate(dateTime)) {
      ...
      // Check custom disabled state first (for holidays, closed days, etc.)
      if (widget.isDateDisabled != null) {
        bool isCustomDisabled = widget.isDateDisabled!.call(dateTime);
        if (isCustomDisabled) {
  ...
  void _init(DatePickerController controller, double ttlWidth, double width, DateTime startDate, DateTime endDate, DateTime selectedDate) {
    ...
    _itemCount = endDate.difference(startDate).inDays;
  }
