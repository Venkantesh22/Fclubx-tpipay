...
Future<List<ReviewData>> employeeReviews({
  ...
  try {
    String employeeId = empId != 0 ? '&${APIParamKeysConst.employeeId}=$empId' : '';
    String branchId = branId != 0 ? '${APIParamKeysConst.branchId}=$branId' : '';

    EmployeeReviewResponse res = EmployeeReviewResponse.fromJson(await handleResponse(await buildHttpResponse(
      '${APIEndPoints.getRating}?$branchId$employeeId&${APIParamKeysConst.perPage}=$perPage&${APIParamKeysConst.page}=$page',
