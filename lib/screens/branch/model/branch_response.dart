...
class BranchData {
  ...
  ///LOCAL
  WorkingHourList? get todayTime => workingHourList.validate().isNotEmpty ? workingHourList!.firstWhere((element) => element.day.validate().toLowerCase().getWeekDayCount == DateTime.now().weekday.validate()) : null;
...
class WorkingHourList {
  ...
  WorkingHourList({this.day, this.endTime, this.isHoliday, this.startTime, this.branchBreaks,this.isFestival});
  ...
  factory WorkingHourList.fromJson(Map<String, dynamic> json) {
    return WorkingHourList(
      ...
      isFestival: json['is_festival_holiday'],
    );
  }
...
class AvailableSlot {
  ...
  AvailableSlot({this.value, this.label, this.disabled});
