...
class _UserWalletBalanceScreenState extends State<UserWalletBalanceScreen> {
  ...
  List<PaymentData> payments = getPaymentList(includeWallet: false, includeCash: false);
    ...
    try {
      if (selectedPayment.id! == PaymentMethods.PAYMENT_METHOD_STRIPE) {
        ...
        razorPayService.init(
          ...
          onComplete: (res) async {
            log(res);
            Map req = {"amount": walletAmountCont.text.toDouble(), "transaction_type": PaymentMethods.PAYMENT_METHOD_RAZORPAY, "transaction_id": res['transaction_id']};
            await walletTopUpApi(request: req);
            walletAmountCont.text = "0";
          },
        );
        ...
        flutterWaveServices.checkout(
          ...
          isTestMode: true,
          onComplete: (res) async {
            Map req = {"amount": walletAmountCont.text.toDouble(), "transaction_type": PaymentMethods.PAYMENT_METHOD_FLUTTER_WAVE, "transaction_id": res['transaction_id']};
            await walletTopUpApi(request: req);
            walletAmountCont.text = "0";
          },
        );
                ...
                children: [
                  ...
                  Container(
                    ...
                    decoration: boxDecorationWithRoundedCorners(
                      ...
                      border: Border.all(color: appStore.isDarkMode ? dividerDarkColor : indicatorColor, width: 1),
                    ),
                    child: Row(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                children: [
                                  Text(locale.availableBalance, style: secondaryTextStyle(size: 14)),
                                ],
                              ),
                              Observer(
                                builder: (context) {
                                  return PriceWidget(price: appStore.userWalletAmount, size: 32, isBoldText: true, color: appStore.isDarkMode ? Colors.white : black);
                                }
                              ),
                            ],
                          ),
                    ...
                    children: [
                      Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            16.height,
                            Text(locale.lblEnterAmount, style: boldTextStyle(size: LABEL_TEXT_SIZE)),
                            8.height,
                            Text(locale.topUpAmountQuestion, style: secondaryTextStyle(size: 12)),
                            AppTextField(
                              textFieldType: TextFieldType.NUMBER,
                              controller: walletAmountCont,
                              focus: walletAmountFocus,
                              textStyle: primaryTextStyle(size: 24, weight: FontWeight.w600),
                              ...
                              children: List.generate(defaultAmounts.length, (index) {
                                return Container(
                                  ...
                                  decoration: boxDecorationDefault(
                                    color: defaultAmounts[index].toString() == walletAmountCont.text ? primaryColor :  (appStore.isDarkMode ? scaffoldSecondaryDark : primaryLightColor),
                                    borderRadius: radius(6),
                                    border: Border.all(color: defaultAmounts[index].toString() == walletAmountCont.text ? context.primaryColor : lavenderColor),
                                  ),
                                  child: PriceWidget(
                                    ...
                                    isBoldText: false,
                                    decimalPoint: 0,
                                    color: defaultAmounts[index] == walletAmountCont.text.toDouble() ? white :  (appStore.isDarkMode ? Colors.white : black),
                                  ),
                                ).onTap(() {
                                  walletAmountCont.text = "${defaultAmounts[index]}";
                                  setState(() {});
                                });
                              }),
                            ),
                      ...
                      Container(
                        ...
                        child: Column(
                          ...
                          spacing: 0,
                          children: [
                            Text(locale.paymentMethod, style: boldTextStyle(size: LABEL_TEXT_SIZE, weight: FontWeight.w600, color: appStore.isDarkMode ? Colors.white : black)),
                            4.height,
                            Text(locale.selectYourPaymentMethodToAddBalance, style: secondaryTextStyle(size: 12, weight: FontWeight.w400)),
                            16.height,
                            AnimatedWrap(
                              listAnimationType: ListAnimationType.None,
                              ...
                              itemBuilder: (context, index) {
                                ...
                                return Container(
                                  decoration: boxDecorationWithRoundedCorners(
                                    borderRadius: BorderRadius.circular(6),
                                    backgroundColor: appStore.isDarkMode ? scaffoldSecondaryDark : scaffoldPrimaryLight,
                                    border: Border.all(color: borderColor, width: 0.5),
                                  ),
                                  child: RadioGroup<PaymentData>(
                                    ...
                                    child: RadioListTile(
                                      ...
                                      fillColor: WidgetStatePropertyAll(selectedPayment == payments[index] ? primaryColor : borderColor),
