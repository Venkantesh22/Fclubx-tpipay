...
class _InvoiceRequestDialogComponentState extends State<InvoiceRequestDialogComponent> {
  ...
  // Optional: request READ permission for older Android versions if needed
  Future<void> _maybeRequestLegacyPermission() async {
    // For Android < 10, WRITE_EXTERNAL_STORAGE may be required; ignore failures gracefully
    try {
  ...
  Future<void> downloadPdf() async {
    try {
      ...
      // Try to request legacy permission (non-blocking). We will save into app sandbox so this isn't mandatory.
      await _maybeRequestLegacyPermission();

      // Build endpoint using shared network utils (ensures auth + localization headers)
      final String endPoint = '${APIEndPoints.orderInvoiceDownload}=${widget.bookingId}';

      final Map<String, dynamic> jsonResponse = await handleResponse(
      ...
      if (invoiceResponse.status && invoiceResponse.link.isNotEmpty) {
        ...
        // Save in app's temporary directory (no special permission required)
        final Directory tempDir = await getTemporaryDirectory();
        final String filePath = '${tempDir.path}/invoice_${widget.bookingId}.pdf';
        ...
        ShowToast.showError(locale.invalidResponseOrLinkNotFound);
      }
    } catch (e) {
      print(e);
      ShowToast.showError('${locale.error}: $e');
    } finally {
      setState(() {
        isLoading = false;
      });
