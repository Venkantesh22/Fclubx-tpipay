...
Future<DashboardResponse> userDashboard({required int branchId, required int perPage}) async {
  String endPoint = '${APIEndPoints.dashboardDetail}?${APIParamKeysConst.branchId}=$branchId&${APIParamKeysConst.perPage}=$perPage';

  // ✅ Add user_id only if available
  if (userStore.userId.validate() != 0) {
    endPoint += '&${APIParamKeysConst.userId}=${userStore.userId}';
  }

  try {
    dashboardResponseCached = DashboardResponse.fromJson(
      await handleResponse(await buildHttpResponse(endPoint, method: HttpMethodType.GET)),
    );

    /// Commented Package snippet for future usage
    // dashboardResponseCached!.expiringPackagesList = await getExpiredPackages();
    // await getPackagesList().then((value) async {
    //   if (value.isNotEmpty) {
    //     await getUserPackagesList().then((val) {
    //       if (val.isNotEmpty) {
    //         val.forEachIndexed((packageData, index) {
