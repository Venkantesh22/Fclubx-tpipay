                  ...
                  return GestureDetector(
                    onTap: () async {
                      ...
                      if (notificationData.data!.notificationDetail!.id.validate() > 0) {
                        final detail = notificationData.data!.notificationDetail!;
                        final String group = detail.notificationGroup.validate().toLowerCase();
                        if (group == "shop") {
                          OrderDetailScreen(orderId: detail.id.validate(), orderCode: detail.orderCode.validate()).launch(context, pageRouteAnimation: PageRouteAnimation.Fade);
                        } else if (group == "wallet") {
                          return;
                        } else {
                          BookingDetailScreen(bookingId: detail.id.validate().toInt()).launch(context);
                        }
                      }
                    },
