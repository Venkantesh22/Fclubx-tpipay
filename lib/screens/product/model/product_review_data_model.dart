...
class ProductReviewDataModel {
  ...
  factory ProductReviewDataModel.fromJson(Map<String, dynamic> json) {
    List<ReviewGallaryData>? reviewGallary;
    if (json['review_gallary'] != null) {
      try {
        // Try to parse as list of objects first
        reviewGallary = (json['review_gallary'] as List).map((i) {
          if (i is String) {
            // If it's a string URL, create a ReviewGallaryData with just the URL
            return ReviewGallaryData(fullUrl: i);
          } else {
            // If it's an object, parse normally
            return ReviewGallaryData.fromJson(i);
          }
        }).toList();
      } catch (e) {
        reviewGallary = null;
