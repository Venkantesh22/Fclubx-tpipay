...
Future<List<EmployeeData>> getEmployeeList({
  ...
  try {
    String order = orderBy.isNotEmpty ? '&${APIParamKeysConst.order_by}=$orderBy' : '';
    String services = serviceIds.isNotEmpty ? '&${APIParamKeysConst.serviceIds}=$serviceIds' : '';
    String dateParam = selectedDate != null ? '&${APIParamKeysConst.date}=$selectedDate' : '';
    String timeParam = selectedTime != null ? '&${APIParamKeysConst.time}=$selectedTime' : '';

    EmployeeResponse res = EmployeeResponse.fromJson(await handleResponse(await buildHttpResponse(
      '${APIEndPoints.employeeList}?${APIParamKeysConst.branchId}=$branchId$order$services$dateParam$timeParam&${APIParamKeysConst.perPage}=$perPage&${APIParamKeysConst.page}=$page',
...
Future<EmployeeDetailResponse> getEmployeeDetail({required int employeeId}) async {
  try {
    var res = EmployeeDetailResponse.fromJson(await handleResponse(
        await buildHttpResponse('${APIEndPoints.employeeDetail}?${APIParamKeysConst.employeeId}=$employeeId', method: HttpMethodType.GET)));
